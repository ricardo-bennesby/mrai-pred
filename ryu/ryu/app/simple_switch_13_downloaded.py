...
class SimpleSwitch13(app_manager.RyuApp):
	...
	self.as_path_list_str = ""
        ...
        if self.num_packets == 450:
	    for neighbor_ip in self.neighborIP:
	    	msg = "neighbor " + neighbor_ip + " announce route 122.150.226.0/24 next-hop self origin egp as-path [65008]\n"
	    	self.logger.info("Sending announcement to neighbor %s", neighbor_ip)
            	self.sendMessage(msg, neighbor_ip)

        if self.num_packets == 500:
	    msg = "neighbor " + "100.78.128.116" + " announce route 122.150.226.0/24 next-hop self origin egp as-path [65008]\n"
	    self.logger.info("Sending announcement to neighbor %s", "100.78.128.116")
            self.sendMessage(msg, "100.78.128.116")
	...
	print('[2] Source MAC : ' + self.eth_addr(packet[6:12]) + ' - Destination MAC : ' + self.eth_addr(packet[0:6]) + ' Protocol : ' + str(eth_protocol))
	...
	self.mac_dst = self.eth_addr(packet[0:6])
	    ...
	    print('[3] IP Header Length : ' + str(ihl) + ' Protocol : ' + str(protocol) + ' Source Address : ' + str(s_addr) + ' Destination Address : ' + str(d_addr))
 #TCP protocol
            if protocol == 6 :
                ...
                print('[4] Source Port : ' + str(source_port) + ' Dest Port : ' + str(dest_port) + ' Sequence Number : ' + str(sequence) + ' Acknowledgement : ' + str(acknowledgement) + ' TCP header length : ' + str(tcph_length))    
    ...
    def sendMessage(self,msg,neighbor_ip):
        self.logger.info("sendMessage called!")
        tcp = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        #dest = (neighbor_ip, port)
	dest = ("192.168.1.151", port)
        tcp.connect(dest)
