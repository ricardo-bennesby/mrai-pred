...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions)]
        if buffer_id:
            #self.logger.debug("There is buffer_id")
            mod = parser.OFPFlowMod(datapath=datapath, buffer_id=buffer_id,
                                    priority=priority, match=match,
                                    instructions=inst)
        else:
            #self.logger.debug("buffer_id==None")
    ...
    def _packet_in_handler(self, ev):
        ...
        #self.logger.debug("packet size in event %s and total length: %s",
        #                      ev.msg.msg_len, ev.msg.total_len)
        ...
        #if ip== None:
         #   self.logger.info("packet in dpid: %s; MAC src: %s; MAC dst: %s; in_port: %s", dpid, src, dst, in_port)
 ...
 #TCP protocol
            if protocol == 6 :
                ...
                if len(self.remaining_buffer) > 0:
                    ...
                    self.logger.info("has_buffer_from_previous_packet")
