...
tcp_ip = 'localhost'
tcp_port = 10000
message1 = "[1,4,2,1,5654,645,34,2,12]"
message2 = "[36,6,7,3,23]"
...
class SimpleSwitch13(app_manager.RyuApp):
	...
	self.parse_new_tcp_seq = 0
	self.current_time_seconds = 0
	self.unique_announcements = []
	self.total_aspath = 0
	self.num_updates = 0
	self.as1 = 5719
	self.as2 = 198507
	...
	self.as4 = 198507	
	self.as5 = 2603
	self.tcp_sequence_set = set()
	#self.mrai = 15

	#features
	self.num_announcements = 0
	self.num_withdrawals = 0
	self.num_duplicate_announcements = 0
	self.longest_aspath = 0
	self.shortest_aspath = 10
	self.avg_as_path = 0.0
	self.num_prepended = 0
	self.prepended_as_set = set()
	self.number_prepended_ases = 0
	self.unique_as_set = set()
	self.stored_announcements_set = set()
	self.total_edit_distance_list = []
	self.tlong_events = 0
	self.tshort_events = 0
	self.avg_edit_distance = 0.0
	self.maximum_edit_distance = 0
	self.num_edited_paths = 0
	self.number_timestamps = 0
	self.rare_as1 = 0
	self.rare_as2 = 0
	self.rare_as3 = 0
	self.rare_as4 = 0
	self.rare_as5 = 0
	self.event_type = 1 # Tup: 0 ; Tshort: 1; Tlong: 2
	self.features_list = []
	self.features_dataset = []
	self.record_data = 0
	self.list_messages_to_send = []
	self.enable_sending_messages = 0
	...
	self.MESSAGE1 = "[1,4,2,1,5654,645,34,2,12]"
	self.MESSAGE2 = "[36,6,7,3,23]"
	...
	#if self.num_packets == 100:
	#	message1 = "neighbor 100.78.128.1 announce route 184.164.226.0/24 next-hop self origin egp as-path [47065]"
	#	self.messageKeras(message1)
	#	time.sleep(1)
	#	message2 = "neighbor 100.78.128.1 announce route 184.164.226.0/24 next-hop self origin egp as-path [47065 47065]"
	#	self.messageKeras(message2)
		

        #if self.num_packets == 1900:
        #    msg = "neighbor 100.78.128.1 announce route 184.164.226.0/24 next-hop self origin egp"
        #    self.sendMessage(msg)

	  #if len(self.accepted_prefixes_set) == 20000:
      	#msg = "neighbor 100.78.128.1 announce route 184.164.227.0/24 next-hop self origin egp as-path [47065]"
		#self.logger.info(msg)
            #self.sendMessage(msg)

        if len(self.accepted_prefixes_set) == 42000:
            msg = "neighbor 100.78.128.1 announce route 184.164.226.0/24 next-hop self origin egp as-path [47065 47065]"
            #self.sendMessage(msg)
		#self.messageKeras(msg)
 ...
 #TCP protocol
	    #if str(s_addr) == "100.78.128.110":
	    #	self.logger.info("****************** BGP Identifier: %s", str(s_addr))
	    #	self.parse_new_tcp_seq = 1
		...
		if str(s_addr) == "100.78.128.110": 
			if str(dest_port)=="179" or str(source_port)=="179":
	    			self.logger.info("****************** BGP Identifier - part 1: %s", str(s_addr))
				#self.parse_new_tcp_seq = 1
				self.parse_packets_external_vm(packet)
				return
			#else:
			#	self.logger.info("********* str(s_addr) == 100.78.128.110 and port != 179")
			#	return
		#else:
			#self.parse_new_tcp_seq = 0
		    ...
		    #if self.parse_new_tcp_seq != 1:
                    self.seq_number_set.add(sequence)
                    	#self.logger.info("self.parse_new_tcp_seq != 1 and self.buffer_dictionary: %d",len(self.buffer_dictionary))

		    #if self.parse_new_tcp_seq == 1:
	 	    #	self.logger.info("Will parse new TCP packets...")
		    #	self.logger.info("self.remaining_buffer: %s",self.remaining_buffer_type)
                ...
                while packet_total_len < len(packet)-14:
                    ...
                    if b+bgp_iterator + 19 > len(packet) or (len(packet)-14 - ip_len < 19) :                 
                        ...
                        if self.remaining_buffer_type != 'attribute' and self.remaining_buffer_type != 'nlri':
                            ...
                            return
                    #if len(self.remaining_buffer) > 0:
                    if self.remaining_buffer_type == 'header' and self.parse_new_tcp_seq != 1:
				    ...
				    if str(bgp_identifier) == "100.78.128.110":
				    	self.logger.info("****************** self.parse_new_tcp_seq <= 0")
					self.parse_new_tcp_seq = 0
					self.bgp_message_sum = self.bgp_message_sum - 19
					return
				    ...
				    if int(bgpunfeasiblelen2[0]) > 0:
					self.num_withdrawals = self.num_withdrawals + 1
					self.logger.info("self.num_withdrawals: %d", self.num_withdrawals)
				    ...
				    if self.bgp_total_path_att_len> 0:
					self.num_announcements = self.num_announcements + 1
					#self.logger.info("self.num_announcements: %d", self.num_announcements)
				...
				if self.bgp_total_path_att_len> 0:
					...
					if pathAtt[2] == 0:
						...
						#self.logger.info("Attribute Length: %d", pathAtt[2])                              
                                        	...
                                        	#Segment Length:      
                                        	attribute_value = self.path_attributes[self.total_path_attribute_len+5:self.total_path_attribute_len+6]
                                        	attributeValue = unpack('!B',attribute_value)                                       
                                        	attributeValue1 = str(attributeValue).split('(')
                                        	attributeValue2 = str(attributeValue1[1]).split(',')
						...
						aspath_list = []
 						edit_distance_list = []
                                        	while num_ASes < int(attributeValue2[0]):
                                            		as_sequence_value = self.path_attributes[self.total_path_attribute_len+6+cont:self.total_path_attribute_len+10+cont]
							...
							if int(as_sequenceValue2[0]) == self.as1:
								self.rare_as1 += 1
							elif int(as_sequenceValue2[0]) == self.as2:
								self.rare_as2 += 1 
							elif int(as_sequenceValue2[0]) == self.as3:
								self.rare_as3 += 1
							elif int(as_sequenceValue2[0]) == self.as4:
								self.rare_as4 += 1
							elif int(as_sequenceValue2[0]) == self.as5:
								self.rare_as5 += 1
							str_as_path = str_as_path + str(int(as_sequenceValue2[0])) + " "
							aspath_list.append(int(as_sequenceValue2[0]))
					    		self.as_path_list_str = self.as_path_list_str + str(int(as_sequenceValue2[0])) + " "
                                            		num_ASes = num_ASes + 1 
                                            		cont = cont + 4 #Each AS has 4 Bytes
						if len(self.accepted_prefixes_set) > 751200:
							current_time = datetime.now().strftime("%H:%M:%S")
								...
								self.logger.info("## started time couting ##")						
					    		#self.logger.info("elapsed time: %s",str(timestamp-self.start_time))
							#self.logger.info("timestamp: %s",str(timestamp))
							#self.logger.info("self.start_time: %s",str(self.start_time))
							if timestamp-self.start_time > 10:
								self.logger.info("## elapsed 10 seconds ##")
								self.logger.info("Timestamp: %s",str(current_time))
								self.start_time = timestamp

								self.logger.info("Number of announcements: %d", self.num_announcements)
								self.logger.info("Number of withdrawals: %d", self.num_withdrawals)
								self.logger.info("Number of duplicate paths: %d", self.num_duplicate_announcements)
								self.logger.info("self.longest_aspath: %d", self.longest_aspath)
								self.logger.info("self.shortest_aspath: %d", self.shortest_aspath)
								self.logger.info("self.avg_as_path: %d", self.avg_as_path)
								self.logger.info("self.number_prepended_ases: %d", self.number_prepended_ases)
								self.logger.info("self.tlong_events: %d", self.tlong_events)
								self.logger.info("self.tshort_events: %d", self.tshort_events)
								self.logger.info("self.avg_edit_distance: %d", self.avg_edit_distance)
								self.logger.info("self.maximum_edit_distance: %d", self.maximum_edit_distance)
								self.logger.info("self.num_edited_paths: %d", self.num_edited_paths)

								self.features_list = []
								self.features_list.append(self.num_announcements)
								self.features_list.append(self.num_withdrawals)
								self.features_list.append(self.num_duplicate_announcements)
								self.features_list.append(self.longest_aspath)
								self.features_list.append(self.shortest_aspath)
								self.features_list.append(self.avg_as_path)
								self.features_list.append(self.number_prepended_ases)
								self.features_list.append(self.tlong_events)
								self.features_list.append(self.tshort_events)
								self.features_list.append(self.avg_edit_distance)
								self.features_list.append(self.maximum_edit_distance)
								self.features_list.append(self.num_edited_paths)
								self.features_list.append(self.rare_as1)
								self.features_list.append(self.rare_as2)
								self.features_list.append(self.rare_as3)
								self.features_list.append(self.rare_as4)
								self.features_list.append(self.rare_as5)
								self.features_list.append(self.event_type)
								self.features_dataset.append(self.features_list)
								...
								self.rare_as4 = 0
								self.rare_as5 = 0
								self.number_timestamps = self.number_timestamps + 1
								self.logger.info("self.number_timestamps: %s",str(self.number_timestamps))	
							if str_as_path not in self.unique_announcements:
								self.unique_announcements.append(str_as_path)
							else:
								self.num_duplicate_announcements = self.num_duplicate_announcements + 1
							#self.duplicate_announcements.add(str_as_path)
							#self.logger.info("Number of duplicate paths: %d", self.num_duplicate_announcements)
							if int(attributeValue2[0]) > self.longest_aspath:
								self.longest_aspath = int(attributeValue2[0])
							if int(attributeValue2[0]) < self.shortest_aspath:
								self.shortest_aspath = int(attributeValue2[0])
							#self.logger.info("Longest AS-PATH: %d", self.longest_aspath)	
							#self.logger.info("Shortest AS-PATH: %d", self.shortest_aspath)
							self.num_updates = self.num_updates + 1
							self.total_aspath = self.total_aspath + int(attributeValue2[0])
							self.avg_as_path = float(self.total_aspath)/float(self.num_updates) 
							#self.logger.info("Average AS-PATH: %f", self.avg_as_path) 
							for as_ in aspath_list:
								#distinct_ases.add(as_)
								if as_ in self.prepended_as_set:
									self.number_prepended_ases = self.number_prepended_ases + 1
									#self.logger.info("Number of prepended: %s",str(self.number_prepended_ases))
								self.prepended_as_set.add(as_)
							self.prepended_as_set = set()
							present_path = 0
							as_path_list = str_as_path.split(' ')
							#print("as_path_list:")
							print(as_path_list)
							for as_path in self.stored_announcements_set:
								stored_as_path = as_path.split(' ')
								if stored_as_path[len(stored_as_path)-2] == as_path_list[len(as_path_list)-2]:
									unique_path_set.remove(as_path)
									unique_path_set.add(str_as_path)
									present_path = 1
									#self.logger.info("****************************************************")
									#self.logger.info("Detected path to edit:")
									#print("Replace \'"+as_path+"\' by \'"+str_as_path+"\'")
									if len(as_path_list) > len(stored_as_path):
										...
										for as_index in range(1,len(stored_as_path)-2):
										...
										self.total_edit_distance_list.append(edit_distance)
										self.tlong_events += 1
										#print "Tlong detected"
										...
										num_path_mods = 0
										#In the following line we take the length of the shorter router to compare ASes, excluding the first and last dest AS_nums
										for as_index in range(1,len(as_path_list)-2):
											if as_path_list[as_index] != stored_as_path[as_index]:
												num_path_mods += 1
										edit_distance = num_path_mods+path_len_difference
										#print "EDIT DISTANCE: " + str(edit_distance)
										...
										if edit_distance > 0:
											...
											self.total_edit_distance_list.append(edit_distance)
									break
							if len(self.stored_announcements_set) == 0 or present_path == 0:
								self.stored_announcements_set.add(str_as_path)
							#print("Edit distance list:")		
							print(self.total_edit_distance_list)
							#self.num_edited_paths = 0
							if len(edit_distance_list) == 0:
								self.avg_edit_distance = float(0) + self.avg_edit_distance
								self.maximum_edit_distance = 0 + self.maximum_edit_distance
							else:
								if len(self.total_edit_distance_list) > 0: 
									self.maximum_edit_distance = max(self.total_edit_distance_list)
								for edit_element in edit_distance_list:
									if edit_element != 0:
										self.num_edited_paths+=1
								if self.num_edited_paths == 0:
									self.avg_edit_distance = float(sum(self.total_edit_distance_list))
								else:
									self.avg_edit_distance = float(sum(self.total_edit_distance_list))/float(self.num_edited_paths)
							#print("Average Edit distance:")
							print(self.avg_edit_distance)
							#print("maximum_edit_distance:")
							print(self.maximum_edit_distance)
							#print("Number of edited paths:")
							print(self.num_edited_paths) 
						...
						if len(self.accepted_prefixes_set) > 751200:
							...
							#self.logger.info("self.start_time: %s",str(self.start_time))
							if timestamp-self.start_time > 10:
								self.logger.info("## elapsed 1 second ##")
								self.logger.info("Timestamp: %s",str(current_time))
								self.start_time = timestamp
							...
							for as_ in aspath_list:
								...
								if as_ in self.prepended_as_set:
									self.number_prepended_ases = self.number_prepended_ases + 1
									#print("Number of prepended: ",str(self.number_prepended_ases))
								self.prepended_as_set.add(as_)
							self.prepended_as_set = set()
							present_path = 0
							as_path_list = str_as_path.split(' ')
							...
							for as_path in self.stored_announcements_set:
								stored_as_path = as_path.split(' ')
								if stored_as_path[len(stored_as_path)-2] == as_path_list[len(as_path_list)-2]:
									self.stored_announcements_set.remove(as_path)
									self.stored_announcements_set.add(str_as_path)
									present_path = 1
									#self.logger.info("****************************************************")
									#self.logger.info("Detected path to edit:")
					...
					self.regular_att_len = 0       
					    ...
					    minutes = int(current_time_list[1])*60
					    timestamp = hours + minutes + int(current_time_list[2])
					    if len(self.accepted_prefixes_set) >= 747400:
					    	if self.start_time == 0:
					    		self.start_time = timestamp						
					    	#self.logger.info("elapsed time: %s",str(timestamp-self.start_time))
						if timestamp-self.start_time >= 300:
					    	...
					    	#self.logger.info("time: %s",str(datetime.now()))	
                                            if int(nlriValue2[0]) <= 24:
                                            ...
                                            if str(nlri_split[3]) == "255" and nlriNextByte == "('\xff',)" and nlriValueLen3 == 1:
                                                ...
                                                #self.logger.info("self.total_number_prefixes: %d",self.total_number_prefixes)
                                                self.accepted_prefixes_set.add(nlri_str)
						if nlri_str == "184.164.227.0/24":
							current_time = datetime.now().strftime("%H:%M:%S")
							#current_time_list = current_time.split(":")
							#self.logger.info("current_time_list[2]: %s",current_time_list[2])
							to_print = self.as_path_list_str + "\n" + current_time
							f = open('/home/ubuntu/update_file.txt','a')
	    						f.write(to_print)
							f.write('\n')
	    						f.close()
							self.logger.info("Time 184.164.227.0/24 prefix arrived: %s",current_time)
							#msg = "neighbor 100.78.128.1 announce route 184.164.227.0/24 next-hop self origin egp  as-path [47065 47065]"
            						#self.sendMessage(msg) 
						#if len(self.accepted_prefixes_set) > 745000:
							#self.logger.info("NLRI: %s",nlri_str)
                                                self.logger.info("Number of accepted prefixes: %d",len(self.accepted_prefixes_set))
                                                #self.logger.info("------------------------------------------")
                                            else:
                                                if nlriValueLen3 == 1:
							...
							#msg = "neighbor 100.78.128.1 announce route 184.164.227.0/24 next-hop self origin egp  as-path [47065 47065]"
						    	...
						    	#self.logger.info("NLRI: %s",nlri_str)
                                                    ...
                                                    self.logger.info("Number prefixes: %d",len(self.accepted_prefixes_set))
						    if len(self.accepted_prefixes_set) == 30000:
            					    	msg = "neighbor 100.78.128.1 announce route 184.164.226.0/24 next-hop self origin egp  as-path [47065]"
            						#self.sendMessage(msg) 

						    if len(self.accepted_prefixes_set) == 52000:
            					    	msg = "neighbor 100.78.128.1 announce route 184.164.226.0/24 next-hop self origin egp  as-path [47065 47065]"
            						#self.sendMessage(msg)
                                                    #self.logger.info("------------------------------------------")
						    ...
						    if len(self.accepted_prefixes_set) == 52000:
            					    	msg = "neighbor 100.78.128.1 announce route 184.164.226.0/24 next-hop self origin egp  as-path [47065 61574]"
            						#self.sendMessage(msg)
                                                    #self.logger.info("------------------------------------------")
    ...
    def parse_packets_external_vm(self,packet):
    	#parse ethernet header
        eth_length = 14
	...
	self.mac_dst = self.eth_addr(packet[0:6])
		...
		if tcp_data_len < 19:
			return               
                ...
                if packet_total_len < len(packet)-14:                    
                    type_unfeasible_len_before_buffer_len_zero = 0 #Used to update bgp_iterator after NLRI parse when remaining_buffer == 0  
							...
							if pathAtt[2] == 0:
								...
								has_destination = 0
                                        			while num_ASes < int(attributeValue2[0]):
                                            				...
                                            				as_sequenceValue2 = str(as_sequenceValue1[1]).split(',') 
                                            				self.logger.info("AS: %d", int(as_sequenceValue2[0]))
									if int(as_sequenceValue2[0]) == 65015:
										has_destination = 1
										self.logger.info("Len AS-PATH: %d", int(attributeValue2[0]))
					    				self.as_path_list_str = self.as_path_list_str + str(int(as_sequenceValue2[0])) + " "
                                            				num_ASes = num_ASes + 1 
                                            				cont = cont + 4 #Each AS has 4 Bytes
								len_ases = int(attributeValue2[0])-1
								if has_destination == 1:
									for i in range(0,len_ases):
										str_as_list = str_as_list + "47065 "
 
									msg = "neighbor 100.78.128.1 announce route 184.164.227.0/24 next-hop self origin egp as-path [ " + str_as_list +"]"
									self.logger.info("msg: %s",msg)

									str_as_list2 = ""
									for i in range(0,len_ases-1):
										str_as_list2 = str_as_list2 + "47065 "
									msg2 = "neighbor 100.78.128.1 announce route 184.164.227.0/24 next-hop self origin egp as-path [ " + str_as_list2 +"]"
									self.logger.info("msg2: %s",msg2)
									
									self.list_messages_to_send = []
									self.list_messages_to_send.append(msg)
									self.list_messages_to_send.append(msg2)
            								#self.sendMessage(msg)
									print "...... FEATURES DATASET: ........."
									for i in range(0,len(self.features_dataset)):
										print self.features_dataset[i]
										print "\n"
            								#self.sendMessage(msg)
									#if self.record_data == 0:
										#self.record_data = 1
										#self.sendMessage(msg)
									#elif self.record_data == 1:
									#write data to file
									new_features_list = self.features_dataset[-80:]
									dataset_file = open("features_dataset.txt","w")
									self.logger.info("WRITE TO FILE!!")
									#str_to_send = ""
									for item in new_features_list:
										#str_to_send = str_to_send + str(item) + '\n'
        									dataset_file.write("%s\n" % item)
									dataset_file.close()
									self.record_data = 0
									#time.sleep(1)
									if str(sequence) not in self.tcp_sequence_set:
										if len(self.list_messages_to_send) > 1 and self.enable_sending_messages == 0:
											for announce_msg in self.list_messages_to_send:
												self.tcp_sequence_set.add(str(sequence))
												#time.sleep(self.mrai)
												time.sleep(1)
												self.logger.info("$$$$$$ SEND MESSAGE")
												#self.sendMessage(announce_msg)
												self.messageKeras(announce_msg)
												#self.enable_sending_messages = 1
												self.list_messages_to_send = []
											del self.list_messages_to_send[:]
    ...
    def messageKeras(self,msg):
        self.logger.info("sendMessage keras!")
        tcp = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        dest = (tcp_ip, tcp_port)
        tcp.connect(dest)
        self.logger.info("Message sent to Keras App...")
        tcp.send (msg)
