...
class SimpleSwitch13(app_manager.RyuApp):
	...
	test_train_file = "test-train.csv"
	csv_after = open(test_train_file,"w")
 ...
 #TCP protocol
		...
		if str(s_addr) == "100.78.128.110": 
			...
			self.logger.info("self.remaining_buffer: %s",self.remaining_buffer_type)
                    ...
                    elif self.remaining_buffer_type !={} or self.enable_check_next_tcp_sequence == 1:
                ...
                while packet_total_len < len(packet)-14:
                    ...
                    if self.remaining_buffer == {} or self.remaining_buffer_type == 'nlri' or self.remaining_buffer_type == 'attribute' or self.remaining_buffer_type == 'unfeasible' or self.remaining_buffer_type == 'update_header' or self.parse_new_tcp_seq == 1:
			if self.parse_new_tcp_seq == 1:
				self.logger.info("**********self.parse_new_tcp_seq == 1...")
				self.logger.info("self.remaining_buffer: %s",self.remaining_buffer_type)
				#self.parse_new_tcp_seq = 0
                        if self.remaining_buffer_type != 'nlri' or self.parse_new_tcp_seq == 1:
                            if self.remaining_buffer_type != 'attribute' and self.remaining_buffer_type != 'update_header' and self.remaining_buffer_type != 'unfeasible' or self.parse_new_tcp_seq == 1:    
				...
				if self.parse_new_tcp_seq == 1 and self.bgp_type!= 1:
					self.parse_new_tcp_seq = 0
					return
