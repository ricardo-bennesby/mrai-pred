...
class extendedtcp(packet_base.PacketBase):
    ...
    def __init__(self, src_port=1, dst_port=1, seq=0, ack=0, offset=0,
        ...
        super(extendedtcp, self).__init__()
    ...
    def parser(cls, buf):
        ...
        bits = bits & 0x3f
        length = offset * 4
        if length > extendedtcp._MIN_LEN:
            option = buf[extendedtcp._MIN_LEN:length]
        else:
            option = None
    ...
    def serialize(self, payload, prev):
        ...
        h = bytearray(struct.pack(
            extendedtcp._PACK_STR, self.src_port, self.dst_port, self.seq,
