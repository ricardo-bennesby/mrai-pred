...
for peer_id_index in range(0,len(list_peers)):
	...
	dataset_y = read_csv('times_after-'+peers_list[peer_id]+'.csv')
	...
	for index in range(0,int(num_timestamps)-1):
		...
		for j in range(0,int(len(base_time)/2)):
			...
			for num_array in range(0,len_y_sequences):
				...
				if datetime.strptime(new_y_timestamps[num_array+last_index], FMT) < datetime.strptime(base_time[j*2], FMT):
					...
					#print("base_time[j*2]): "+str(base_time[j*2]))
	...
	#print("training_x after delete column 1 shape:")
	#print(len(training_x))
	...
	model.fit(X, scaledY, epochs=num_epochs, batch_size=int(len(training_x)/2), validation_data=(X, scaledY), verbose=2, shuffle=False)
